...
def generate_summary_tables():
    """
    Genera las dos tablas finales listas para incluir en el TFM.
    """
    try:
        studies_df = pd.read_csv(STUDIES_CSV, encoding='utf-8-sig')
        biomark_df = pd.read_csv(BIOMARK_CSV, encoding='utf-8-sig')

        # Tabla de características generales
        study_summary = studies_df[[
            'article_id', 'article', 'journal',
            'study_design', 'subgroups', 'techniques'
        ]].copy()
        study_summary.columns = [
            'ID', 'Artículo', 'Revista',
            'Diseño del estudio', 'Subgrupos de pacientes', 'Técnicas utilizadas'
        ]

        # Tabla de biomarcadores
        bm_summary = biomark_df[[
            'article_id', 'biomarker_name', 'biomarker_type',
            'sample_type', 'clinical_purpose', 'validation_status',
            'main_findings', 'limitations'
        ]].copy()
        bm_summary.columns = [
            'ID artículo', 'Biomarcador', 'Tipo',
            'Muestra', 'Propósito clínico', 'Validación',
            'Hallazgos principales', 'Limitaciones'
        ]

        # Exportar
        study_summary.to_csv(
            os.path.join(BASE_DIR, "resumen_estudios.csv"),
            index=False, encoding='utf-8-sig')
        bm_summary.to_csv(
            os.path.join(BASE_DIR, "resumen_biomarcadores.csv"),
            index=False, encoding='utf-8-sig')
        ...
        return study_summary, bm_summary
